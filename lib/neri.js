...
/**
 * 开播前的口令核验：用 joinSecret 加入一次房间，成功后立刻退出。
 * 并在退出前精确提取 state.track 中的当前播放曲目（歌名 - 歌手）。
 */
export async function verifyRoomSecret(serverUrl, roomId, secret) {
  ...
  try {
    ...
    // 🎵 从实际返回的 state.track 中精确提取歌名和歌手
    let currentSong = null;
    const track = result.state?.track;
    if (track && track.name) {
      currentSong = track.artist ? `${track.name} - ${track.artist}` : track.name;
    }
